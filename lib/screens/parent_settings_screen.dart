...
class _ParentSettingsScreenState extends State<ParentSettingsScreen> {
  ...
  // ── Profile state ──────────────────────────────────────────────────────
  String _profileName = ''; // سيتم جلبه من الداتابيز
  ...
  void initState() {
    ...
    _loadUserData(); // جلب البيانات عند بدء التشغيل
  }
 ...
 void _handleProfileUpdate() async {
    ...
    try {
      ...
      if (uid == null) {
        print("❌ خطأ: لم يتم العثور على UID للمستخدم");
        return;
      }
      ...
      print("✅ تم التحديث في Firestore بنجاح!");
  ...
  void _handlePasswordChange() async {
    ...
    if (newPass != confirmPass) {
      setState(() { _passwordMessage = 'كلمات المرور غير متطابقة'; _isPasswordMessageError = true; });
      return;
    }
    if (newPass.length < 8) {
      setState(() { _passwordMessage = 'يجب أن تكون كلمة المرور ٨ خانات على الأقل'; _isPasswordMessageError = true; });
      return;
    }

    setState(() => _isPasswordLoading = true);
    try {
      // ملاحظة: تغيير الباسورد يتطلب تسجيل دخول حديث (Recent Login) لأسباب أمنية
      ...
      setState(() {
        ...
        _passwordMessage = e.code == 'requires-recent-login' 
            ? 'يرجى تسجيل الخروج والدخول مرة أخرى لتغيير كلمة المرور'
          ...
          child: Row(
            children: [
              ...
              const Column(
                ...
                children: [
                  Text('الإعدادات', style: TextStyle(fontSize: 19, fontWeight: FontWeight.bold, color: Colors.white)),
                  Text('إدارة الحساب والتفضيلات', style: TextStyle(fontSize: 12, color: Colors.white70)),
                ],
              ),
            ],
          ),
  ...
  Widget _buildFullButton(String label, VoidCallback onTap, bool isLoading) {
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(backgroundColor: _pink, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
        child: isLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : Text(label),
      ),
    );
  }
  ...
  Widget _passwordField(TextEditingController controller, String hint, bool show, VoidCallback onToggle) {
    return TextField(
      ...
      decoration: _inputDecoration(hint).copyWith(
        suffixIcon: IconButton(icon: Icon(show ? Icons.visibility_off : Icons.visibility, color: Colors.grey, size: 20), onPressed: onToggle),
      ),
    );
  }
  ...
  InputDecoration _inputDecoration(String hint) {
    return InputDecoration(
      ...
      focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: _pink, width: 2)),
    );
  }
  ...
  Widget _buildMessage(String message, {required bool isError}) {
    return Container(
      ...
      child: Row(children: [Icon(isError ? Icons.error_outline : Icons.check_circle_outline, color: isError ? Colors.red : Colors.green, size: 18), const SizedBox(width: 8), Expanded(child: Text(message, style: TextStyle(fontSize: 13, color: isError ? Colors.red : Colors.green.shade700)))]),
    );
  }
}